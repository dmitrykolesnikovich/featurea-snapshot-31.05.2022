...
import featurea.desktop.jfx.ResizeType.*
...
class SmartResize private constructor() : TableViewResizeCallback {
    ...
    override fun call(param: TableView.ResizeFeatures<out Any>) =
        resizeCall(param.toTornadoFXFeatures()) { table ->
            if (!isPolicyInstalled(table)) install(
                table
            )
        }
    ...
    companion object {
        ...
        private val policyChangeListener = ChangeListener<Callback<TableView.ResizeFeatures<*>, Boolean>> { observable, _, newValue ->
            ...
            if (newValue == POLICY) install(
                table
            ) else uninstall(table)
        }
...
class TreeTableSmartResize private constructor() : TreeTableViewResizeCallback {
    ...
    override fun call(param: TreeTableView.ResizeFeatures<out Any>) =
        resizeCall(param.toTornadoFXResizeFeatures()) { table ->
            if (!isPolicyInstalled(table)) install(
                table
            )
        }
    ...
    companion object {
        ...
        private var TreeTableView<*>.isSmartResizing: Boolean
            get() = properties[IS_SMART_RESIZING] == true
            set(value) {
                properties[IS_SMART_RESIZING] = value
            }
