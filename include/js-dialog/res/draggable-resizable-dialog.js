	...
	*/
	...
	_onMouseDown = function(evt) {
		...
		if ( !(evt.target === _dialog || evt.target === _dialogTitle || evt.target === _buttons[0]
		        || evt.target === _dialog.querySelector(".mainCanvasDialogContent") || evt.target === _dialog.querySelector("#mainCanvas")))
			return;
	...
	_onMouseMove = function(evt) {
		...
		if ( !(evt.target === _dialog || evt.target === _dialogTitle || evt.target === _buttons[0]
		        || evt.target === _dialog.querySelector(".mainCanvasDialogContent") || evt.target === _dialog.querySelector("#mainCanvas")) && !_isDrag && _resizeMode == '')
			return;
		if (_isDrag) {
			...
			_dialogContent.dispatchEvent(new Event("change")); // IMPORTANT needed by libs/opengl/src/jsMain/kotlin/featurea/opengl/GLCanvas.kt
		}
		else if (_isResize) {
			var dw, dh, w, h;
			if (_resizeMode == 'w') {
		...
		else if (!_isButton && document.fullscreenElement == null) { // just for now todo revert to `else if (!_isButton)`
			var cs, rm = '';
	...
	_setDialogContent = function() {
		...
		_dialogTitle.style.width = (w + 2) + 'px';
	},
