		...
		return this.each(function() {
			...
			var new_ele = $(
				"<div class='timepicker_wrap " + settings.custom_classes + "'>" +
					...
					"</div>");
			if (settings.show_meridian) {
				new_ele.append("<div class='meridian'>" + top_arrow_button + "<div class='mer_tx'><input type='text' class='timepicki-input' readonly></div>" + bottom_arrow_button + "</div>");
			}
			if (settings.reset) {
				new_ele.append("<div><a href='#' class='reset_time'>Reset</a></div>");
			}
			new_ele.append("<div style='text-align: center'><button id='timepicker_wrap_button' style='color: black; padding-top: 16px; outline: none;'>OK</button></div>")
			...
			$(document).on("click", function(event) {
				if (!$(event.target).is(ele_next) && ele_next.css("display")=="block" && !$(event.target).is($('.reset_time'))) {
					if (!$(event.target).is(ele)) {
						set_value(event, /*!is_element_in_timepicki($(event.target))*/false);
					} else {
						var ele_lef =  0;
			...
			*/
			// 2)
			ele[0].open_timepicki = open_timepicki
			ele[0].close_timepicki = close_timepicki
			// <<
